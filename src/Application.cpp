		...
		while (SDL_PollEvent(&ev)) {
			...
			case SDL_MOUSEBUTTONUP: {
				if (mbd.x / squareSize - ev.button.x / squareSize == 1 && ev.button.x >= 0) {
					game.swapStones(mbd.x / squareSize, mbd.y / squareSize, ev.button.x / squareSize, mbd.y / squareSize);
				}
				else if (ev.button.x / squareSize - mbd.x / squareSize == 1 && ev.button.x < window_width) {
					game.swapStones(ev.button.x / squareSize, mbd.y / squareSize, mbd.x / squareSize, mbd.y / squareSize);
				}
				else if (mbd.y / squareSize - ev.button.y / squareSize == 1 && ev.button.y >= 0) {
					game.swapStones(mbd.x / squareSize, ev.button.y / squareSize, mbd.x / squareSize, mbd.y / squareSize);
				}
				else if (ev.button.y / squareSize - mbd.y / squareSize == 1 && ev.button.y < window_height) {
					game.swapStones(mbd.x / squareSize, mbd.y / squareSize, mbd.x / squareSize, ev.button.y / squareSize);
				}
				break;
			}
			case SDL_QUIT:
